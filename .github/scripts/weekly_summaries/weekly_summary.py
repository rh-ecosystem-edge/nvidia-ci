...
def get_commits_since(days: int, file_path: str) -> list[str]:
    """Get all commit hashes that modified the file in the last N days."""
...
def collect_tested_versions(commits: list[str], file_path: str) -> Dict[str, Set[str]]:
    ...
    # Get the file content before the first commit (to compare with)
...
def main():
    """Main entry point."""
    file_path = "workflows/gpu_operator_versions/versions.json"
    days = 7

    logger.info(f"Collecting tested versions from last {days} days")

    commits = get_commits_since(days, file_path)
    logger.info(f"Found {len(commits)} commits to {file_path}")

    if not commits:
        logger.info("No commits found in the last 7 days")
        # Still generate a summary with empty data
    ...
    # Output to stdout (workflow will capture this)
    print(summary)

    # Also write to file for the workflow to commit
