...
var (
	...
	mpsTestConfigMapData      = map[string]string{
...
print(f'MPS Test completed in {end - start:.4f} seconds')
"
		if [ ! $? -eq 0 ]; then
		  exit 1
		fi`,
...
func CreateDevicePluginConfigMap(apiClient *clients.Settings, replicas int, configMapName, configMapNamespace string, renameByDefalt bool) (*configmap.Builder, error) {
	// Define the device plugin configuration
	config := map[string]interface{}{
		"version": "v1",
		"sharing": map[string]interface{}{
			"mps": map[string]interface{}{
				"renameByDefault": renameByDefalt,
