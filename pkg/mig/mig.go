	...
	if TsMonAfterPod <= len(tsPodInfo) {
		...
		for _, podInfo := range tsPodInfo {
			...
			if podInfo.Pod.Exists() {
				...
				ret1 := isRunning(podInfo.Pod, podInfo.Namespace)
...
// 2: Pending
// 3: any other
func isRunning(gpuPod *pod.Builder, namespace string) int {
	...
	if err != nil {
		// pod exists, but is not running — distinguish Failed from Pending/other.
...
func isCompleted(gpuMigPodPulled *pod.Builder, namespace string) bool {
	glog.V(gpuparams.Gpu10LogLevel).Infof("%s", colorLog(colorCyan+colorBold, "isCompleted"))
	err := gpuMigPodPulled.WaitUntilInStatus(corev1.PodSucceeded, nvidiagpu.BurnPodSuccessTimeout)
	if err != nil {
		ret := isFailed(gpuMigPodPulled, namespace)
		Expect(ret).To(BeFalse(), "gpu-burn pod %s/%s is not Failed; status=%+v", namespace, gpuMigPodPulled.Definition.Name, gpuMigPodPulled.Object.Status)
		Expect(err).ToNot(HaveOccurred(), "timeout waiting for gpu-burn pod with MIG in "+
