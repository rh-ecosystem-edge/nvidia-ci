...
def make_request(url: str, params: Dict[str, Any] = None, headers: Dict[str, str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        raise_error(f"Error during request to {url}: {e}")
...
def fetch_file_content(file_path: str) -> str:
    ...
    try:
        ...
        raise_error(f"Error fetching file content for {file_path}: {e}")
...
def get_versions(prefix: str, build_id: str, gpu_version_suffix: str) -> Tuple[str, str]:
    ...
    try:
        ...
        raise_error(f"Error in get_versions: {e}")
...
def get_status_and_time(prefix: str, latest_build_id: str) -> Tuple[str, Any]:
    ...
    try:
        ...
        raise_error(f"Error in get_status_and_time: {e}")
...
def get_job_results(
    ...
    try:
        ...
        if status == "SUCCESS":
            exact_ocp, _ = get_versions(prefix, latest_build, gpu_version_suffix)
            result = TestResults(exact_ocp, gpu_version_suffix, status, job_url, timestamp)
        else:
        ...
        raise_error(f"Error in get_job_results for {prefix}: {e}")
...
def get_all_pr_tests(pr_num: str, ocp_data: Dict[str, List[Dict[str, Any]]]) -> None:
    ...
    try:
        ...
        raise_error(f"Error in get_all_pr_tests for PR {pr_num}: {e}")
...
def retrieve_all_prs(ocp_data: Dict[str, List[Dict[str, Any]]]) -> None:
    ...
    try:
        ...
        raise_error(f"Error in retrieve_all_prs: {e}")
...
def main() -> None:
    ...
    parser.add_argument("--old_data_file", required=True, help="Name of the existing ocp_data.json file")
    ...
    try:
        ...
        logger.info(f"No old data found at {old_data_path}; starting with empty data.")
        old_data = {}
    ...
    save_to_json(local_ocp_data, args.output_dir, existing_data=old_data)
