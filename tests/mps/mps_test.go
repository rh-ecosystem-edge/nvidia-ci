...
var _ = Describe("MPS", Ordered, Label(tsparams.LabelSuite), func() {
	...
	BeforeAll(func() {
		...
		if nvidiaGPUConfig.NFDFallbackCatalogsourceIndexImage != "" {
			...
			glog.V(gpuparams.GpuLogLevel).Infof("Setting flag to create custom NFD operator " +
	...
	Context("MPS With large number of repclicas", Label("mps-with-large-replicas"), func() {
		It("Mps daemon should failed and log error message", Label("mps"), func() {
...
func EnsureAllGpuPodsAreRunning() {
	// Wait for NVIDIA driver pods to stabilize after cluster policy update
	time.Sleep(2 * time.Minute)
	Eventually(func() bool {
		driverPods, err := inittools.APIClient.Pods(GPUOperatorNamespace).List(context.TODO(), metav1.ListOptions{})
		if err != nil {
